...
function custom_support_button($atts) {
    ...
    $output .= '<button class="support-button">応援してるよ</button>';
    if ($atts['count']) {
...
function custom_support_button_enqueue_scripts() {
    wp_enqueue_style('custom-support-button-style', plugin_dir_url(__FILE__) . 'assets/style.css');
    wp_enqueue_script('custom-support-button-script', plugin_dir_url(__FILE__) . 'assets/script.js', array('jquery'), '1.0', true);
    wp_localize_script('custom-support-button-script', 'custom_support_button_vars', array(
        'thank_you_message' => '応援ありがとうございます！励みになります！'
    ));
}
